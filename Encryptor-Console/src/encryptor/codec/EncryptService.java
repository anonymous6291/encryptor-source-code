...
public class EncryptService implements Service {
    ...
    private final boolean hideFileName;
    private final byte encr[];
    private final Map<File, File> files;

    public EncryptService(File input, FileOperationsCompletedStatus status, int threads, boolean hideFileName, byte encr[]) throws Exception {
        this.input = input.getCanonicalFile();
        outputPath = getEncryptedDirectory(input.getCanonicalFile());
    ...
    public EncryptService(List<File> explicitFileList, FileOperationsCompletedStatus status, int threads, boolean hideFileName, byte encr[]) {
        input = null;
        outputPath = null;
    ...
    private File getEncryptedDirectory(File curr) throws Exception {
    ...
    private void addAllFiles() throws Exception {
        if (explicitFileListGiven) {
            ...
            for (File f : explicitFileList) {
                ...
                addAllFiles0(original, getEncryptedDirectory(original));
            }
        } else {
    ...
    private void startEncryption() throws Exception {
        for (File f : files.keySet()) {
            try {
                ...
                CustomLogger.log(e.getMessage(), CustomLogger.MESSAGE);
            }
            TaskManager.submit(new EncryptFiles(f, files.get(f), status, lock, hideFileName, encr));
        }
    }
    ...
    private class EncryptFiles implements Runnable {
        ...
        protected EncryptFiles(File input, File output, FileOperationsCompletedStatus status, Semaphore lock, boolean hideFileName, byte encr[]) {
            this.input = input;
            this.output = output;
            this.status = status;
            this.lock = lock;
        ...
        public void run() {
            try {
                if (!input.isFile()) {
                    ...
                    CustomLogger.log("Encrypting: ".concat(input.toString()), CustomLogger.NORMAL);
                    Encrypt.encrypt(input, output, hideFileName, encr);
